...
- `release.yml`, the whole release pipeline in one workflow, generated by dist
  ...
  installers → publish the GitHub Release, the Homebrew formula
  (`shed-sh/homebrew-tap`, needs `HOMEBREW_TAP_TOKEN`), the npm package
  (`@shed-sh/shed`, needs `NPM_TOKEN`), and AUR → announce → smoke test.

The AUR and smoke stages are this repository's own sub-workflows, wired into
the pipeline through `publish-jobs` and `post-announce-jobs` in
`dist-workspace.toml`, so they run in order instead of racing the asset
upload:

- `release-aur.yml` — renders `.github/aur/*.tmpl` with the release checksums
...
- `release-smoke.yml` — installs the published release through
  `install-cli.sh` and npm, verifies the published `.sha256` matches its
  artifact, and fails unless the installed binary reports the tagged version.
