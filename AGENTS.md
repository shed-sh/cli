...
- `release-publish.yaml` (generated by dist from `dist-workspace.toml`) —
  builds the four platform archives, the shell installer, the Homebrew formula
  (`shed-sh/homebrew-tap`, needs `HOMEBREW_TAP_TOKEN`), and the npm package
  (`@shed-sh/shed`, needs `NPM_TOKEN`), and publishes the GitHub Release.
- `release-verify.yaml` — checks the generated docs match the binary, verifies
  each published `.sha256` against the artifact beside it, then installs the
  release through `install-cli.sh` and npm and fails the release unless the
  installed binary reports the tagged version.
- `release-aur.yaml` — renders `.github/aur/*.tmpl` with the release checksums
  and pushes `shed-bin` to AUR (needs `AUR_SSH_PRIVATE_KEY`; a no-op until it
  is configured).

`dist init` and `dist generate` write the release workflow to
`.github/workflows/release.yml`, the name dist assumes. This repository renamed
it to `release-publish.yaml`, so after running either command, move the
regenerated file back over `release-publish.yaml` and delete `release.yml`.
Leaving both means two workflows publish the same tag.
...
`install-skills.sh` clones this repository into `~/.shed/skills` and symlinks
`skills/shed` into each coding agent found on the machine, so one canonical
copy serves every agent and rerunning the script (a `git pull`) updates all of
them at once. git is its only requirement.

Install the skill locally with `cp -R skills/shed ~/.claude/skills/shed`, or
from this repository as a plugin (`.claude-plugin/`).
