...
- `release.yml` (generated by dist from `dist-workspace.toml`) — builds the
  four platform archives, the shell installer, the Homebrew formula
...
- `verify-release.yml` — checks the generated docs match the binary, then
  installs the release through `install-cli.sh` and npm and fails the release
  unless the installed binary reports the tagged version.
- `publish-aur.yml` — renders `.github/aur/*.tmpl` with the release checksums
