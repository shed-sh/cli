		...
		Commands: []Command{
			{
				Name:      "deploy",
				Usage:     "[directory]",
				Summary:   "Build the project and run it locally, or send it up with --remote",
				MinArgs:   0,
				MaxArgs:   1,
				UsageLine: "usage: shed deploy [directory]",
				Flags: []Flag{
					{Name: "dry-run", Kind: KindBool, Summary: "Inspect and package, without building or running"},
					{Name: "mock", Kind: KindBool, Summary: "Package and stop, for when Docker is unavailable"},
					{Name: "remote", Kind: KindBool, Summary: "Send the packaged bundle to Shed cloud"},
				...
				Examples: []Example{
					{Command: "shed deploy", Comment: "Build and run this project"},
					{Command: "shed deploy . --dry-run --archive app.tar.gz", Comment: "Package it and look inside"},
					{Command: "shed deploy . --remote --output json", Comment: "Send it up, one JSON result"},
				},
			},
			{
				Name:      "init",
