...
// NewUnresolved returns a resolver that records requested package versions but
// cannot resolve them. It is intended for provider planning tests that must not
// download or execute mise.
func NewUnresolved() *Resolver {
	return &Resolver{
		packages:         make(map[string]*RequestedPackage),
		previousVersions: make(map[string]string),
	}
...
func (r *Resolver) ResolvePackages() (map[string]*ResolvedPackage, error) {
	if r.mise == nil {
		resolvedPackages := make(map[string]*ResolvedPackage, len(r.packages))
		for name, pkg := range r.packages {
			requestedVersion := pkg.Version
			resolvedVersion := pkg.Version
			resolvedPackages[name] = &ResolvedPackage{
				Name:             name,
