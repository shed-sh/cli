...
APP_NAME="shed"
APP_VERSION="0.1.0"
if [ -n "${SHED_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$SHED_DOWNLOAD_URL"
elif [ -n "${INSTALLER_DOWNLOAD_URL:-}" ]; then
    ARTIFACT_DOWNLOAD_URLS="$INSTALLER_DOWNLOAD_URL"
elif [ -n "${SHED_INSTALLER_GHE_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$SHED_INSTALLER_GHE_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/shed-sh/skills/releases/download/v0.1.0"
elif [ -n "${SHED_INSTALLER_GITHUB_BASE_URL:-}" ]; then
    INSTALLER_BASE_URL="$SHED_INSTALLER_GITHUB_BASE_URL"
    ARTIFACT_DOWNLOAD_URLS="${INSTALLER_BASE_URL}/shed-sh/skills/releases/download/v0.1.0"
else
    ARTIFACT_DOWNLOAD_URLS="https://github.com/shed-sh/skills/releases/download/v0.1.0"
fi
if [ -n "${SHED_PRINT_VERBOSE:-}" ]; then
    PRINT_VERBOSE="$SHED_PRINT_VERBOSE"
else
    PRINT_VERBOSE=${INSTALLER_PRINT_VERBOSE:-0}
fi
if [ -n "${SHED_PRINT_QUIET:-}" ]; then
    PRINT_QUIET="$SHED_PRINT_QUIET"
else
    PRINT_QUIET=${INSTALLER_PRINT_QUIET:-0}
fi
if [ -n "${SHED_NO_MODIFY_PATH:-}" ]; then
    NO_MODIFY_PATH="$SHED_NO_MODIFY_PATH"
else
    NO_MODIFY_PATH=${INSTALLER_NO_MODIFY_PATH:-0}
fi
if [ "${SHED_DISABLE_UPDATE:-0}" = "1" ]; then
    INSTALL_UPDATER=0
else
    INSTALL_UPDATER=1
fi
UNMANAGED_INSTALL="${SHED_UNMANAGED_INSTALL:-}"
if [ -n "${UNMANAGED_INSTALL}" ]; then
    NO_MODIFY_PATH=1
    INSTALL_UPDATER=0
fi
AUTH_TOKEN="${SHED_GITHUB_TOKEN:-}"

read -r RECEIPT <<EORECEIPT
{"binaries":["CARGO_DIST_BINS"],"binary_aliases":{},"cdylibs":["CARGO_DIST_DYLIBS"],"cstaticlibs":["CARGO_DIST_STATICLIBS"],"install_layout":"unspecified","install_prefix":"AXO_INSTALL_PREFIX","modify_path":true,"provider":{"source":"cargo-dist","version":"0.32.0"},"source":{"app_name":"shed","name":"skills","owner":"shed-sh","release_type":"github"},"version":"0.1.0"}
EORECEIPT
...
if is_windows_posix && [ -n "${LOCALAPPDATA:-}" ]; then
    RECEIPT_HOME="$LOCALAPPDATA/shed"
else
    RECEIPT_HOME="${XDG_CONFIG_HOME:-$INFERRED_HOME/.config}/shed"
fi
...
The installer for shed 0.1.0

This script detects what platform you're on and fetches an appropriate archive from
https://github.com/shed-sh/skills/releases/download/v0.1.0
...
download_binary_and_run_installer() {
    ...
    for arg in "$@"; do
        case "$arg" in
            ...
            --verbose)
                ...
                ;;
            --no-modify-path)
                say "--no-modify-path has been deprecated; please set SHED_NO_MODIFY_PATH=1 in the environment"
                NO_MODIFY_PATH=1
                ;;
            *)
                OPTIND=1
    ...
    case "$_artifact_name" in 
        "shed-aarch64-apple-darwin.tar.xz")
            ...
            _updater_bin=""
            ;;
        "shed-aarch64-unknown-linux-gnu.tar.xz")
            ...
            _updater_bin=""
            ;;
        "shed-x86_64-apple-darwin.tar.xz")
            ...
            _updater_bin=""
            ;;
        "shed-x86_64-unknown-linux-gnu.tar.xz")
...
select_archive_for_arch() {
    ...
    case "$_true_arch" in 
        "aarch64-apple-darwin")
            _archive="shed-aarch64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            _archive="shed-x86_64-apple-darwin.tar.xz"
            if [ -n "$_archive" ]; then
                echo "$_archive"
                return 0
            fi
            ;;
        "aarch64-unknown-linux-gnu")
            _archive="shed-aarch64-unknown-linux-gnu.tar.xz"
        ...
        "x86_64-apple-darwin")
            ...
            ;;
        "x86_64-unknown-linux-gnu")
            _archive="shed-x86_64-unknown-linux-gnu.tar.xz"
            if ! check_glibc "2" "31"; then
                _archive=""
            fi
            if [ -n "$_archive" ]; then
...
install() {
    ...
    if [ -n "${SHED_INSTALL_DIR:-}" ]; then
        _force_install_dir="$SHED_INSTALL_DIR"
    ...
    say "shed is installed! Run 'shed login' to connect it to your account."
